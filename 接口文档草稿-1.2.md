...
**响应结构 (VO):**
 > 注意：`url` 和 `coverUrl` 均为完整可访问的 URL (http://...)。
...
```JSON
{
  ...
  "name": "旅行.mp4",
  "name": "旅行.mp4",
  "url": "http://oss.../storage/videos/travel.mp4",
  "coverUrl": "http://oss.../storage/covers/travel_cover.jpg",
...
覆盖热点获取、AI脚本辅助、视频生成任务提交与轮询。

**通用枚举定义:**

- `TaskType`: `TEXT_TO_SCRIPT` (写脚本), `TEXT_TO_IMAGE` (绘图), `IMAGE_TO_VIDEO` (图生视频), `TEXT_TO_VIDEO` (文生视频)
...
| `modelName`   | String   | 是       | 模型名称     | 必须存在于 ai_models 表                                      |
...
- **请求方式**: `GET`
- **描述**: 前端轮询此接口获取进度。当商用api传回下载链接时，修改状态并在后端触发下载，继续轮询，下载完成时修改状态，并将下载地址填入videoUrl（此前置空）
- **权限要求**: 登录用户
...
覆盖 AI 模型接入（多供应商 API Key 与参数统一管理）以及全局系统动态参数（存储、调度配置等 Key-Value 形式）的热加载管理支持。不涉及具体执行层引擎的运行维护。
...
- **请求方式**: `GET`
- **描述**: 获取后台配置的所有模型信息。**注意：**出于安全，返回的 `apiConfig` 中的密钥已进行脱敏处理（如显示 `sk-****`）。
- **权限要求**: 超级管理员 (Sa-Token: ADMIN)
...
- **接口路径**: `/api/v1/admin/models`
- **请求方式**: `POST`
- **描述**: 接入新的模型或更新现有配置。请求体中如果携带 `modelId` 则为修改，否则为新增。
- **权限要求**: 超级管理员
...
| **字段名**    | **类型**   | **必填** | **说明**     | **校验规则**                 |
| ------------- | ---------- | -------- | ------------ | ---------------------------- |
| `modelId`     | Long       | 否       | 模型ID       | 更新时携带                     |
| `modelName`   | String     | 是       | 显示名称     | `@NotBlank`                  |
| `modelKey`    | String     | 是       | 调用标识     | 唯一, 必填                   |
| `provider`    | String     | 是       | 提供商       | 必填                         |
| `modelType`   | String     | 是       | 模型类型     | TEXT/IMAGE/VIDEO/AUDIO 必填  |
| `apiEndpoint` | String     | 是       | 接口地址     | 必填                         |
| `apiConfig`   | JSON/Map   | 是       | 核心动态参数 | apiKey 等扩展鉴权与运行参数  |
| `billingMode` | Integer    | 是       | 计费模式     | 必填                         |
| `unitPrice`   | BigDecimal | 否       | 单价         | -                            |
| `isActive`    | Boolean    | 否       | 是否启用     | 默认 true                    |

**响应结构 (VO):** `Long` (返回落库的 modelId)

### 1.4 启用/停用模型

- **接口路径**: `/api/v1/admin/models/{modelId}/status`
- **请求方式**: `PUT`
- **描述**: 快速上下架某个模型（例如服务宕机时紧急停用）。
- **权限要求**: 超级管理员
...
**响应结构 (VO):** `Boolean` (是否成功)

------

## 2. 全局动态参数配置 (Dynamic System Configs)

*采用 Redis 双写热加载，通过不同的 `category` 大类进行分类，存储灵活的 Key-Value 参数*

### 2.1 获取某类系统参数集合

- **接口路径**: `/api/v1/admin/configs/{category}`
- **请求方式**: `GET`
- **描述**: 按分类获取配置详情（直接从 Redis 缓存提取）。
  - `category` 支持: `STORAGE` (存储参数), `SCHEDULING` (调度并发阈值), `TRANSCODING` (系统转码参数预设) 等。
- **权限要求**: 超级管理员

**响应结构 (VO) - 示例 (获取 category=SCHEDULING):**
...
- **接口路径**: `/api/v1/admin/configs/{category}`
- **请求方式**: `PUT`
- **描述**: 批量提交覆盖更新具体的系统分类下的所有 Key-Value 并触发 Redis 热加载刷新。
- **权限要求**: 超级管理员

**请求参数 (DTO):**

期望的 Body 类型为 `Map<String, String>`，即直接丢入 Key-Value 对集合：
...
*注：后端会根据 `category` 动态查找对应的 Validator 策略进行安全拦截（例如校验 endpoint URL 格式）。*

**响应结构 (VO):** `Boolean` (是否全量覆盖更新成功)
...
| `page`     | int      | 否       | 页码 | 1          |
| `size`     | int      | 否       | 每页数量 | 10         |
| `type`     | String   | 否       | 模版类型过滤 | -          |
| `keyword`  | String   | 否       | 关键词模糊匹配名称和标签 | -          |

**响应结构 (Page<TemplateMarketVO>):**
返回包含 `templateId`, `templateName`, `price`, `likeCount`, `usageCount`, `creatorName`, `purchased`, `liked` 等字段的分页对象。

### 1.2 获取模版详情
- **接口路径**: `/api/v1/market/templates/{templateId}`
- **请求方式**: `GET`
- **描述**: 获取单个模版的详细信息，登录状态下会带上已购和点赞状态。
- **权限要求**: 匿名/登录

**响应结构 (TemplateMarketVO):** 同上。

### 1.3 购买/获取模版
- **接口路径**: `/api/v1/market/templates/{templateId}/purchase`
- **请求方式**: `POST`
- **描述**: 购买指定收费模版（扣款）或直接获取免费模版，防重复购买。
- **权限要求**: 登录用户

**响应结构 (Map):**
`{"templateId": "...", "pricePaid": 0.00, "message": "购买成功"}`

---

## 2. 社交互动服务 (Interaction Service)

### 2.1 点赞/取消点赞
- **接口路径**: `/api/v1/interactions/like`
- **请求方式**: `POST`
- **描述**: 对模版进行点赞或取消点赞，支持幂等切换。
- **权限要求**: 登录用户

**请求参数 (InteractionRequest):**
| **字段名** | **类型** | **必填** | **说明** |
| ---------- | -------- | -------- | -------- |
| `targetId` | String   | 是       | 目标模版ID |

**响应结构 (Map):** `{"liked": true, "message": "点赞成功"}`

### 2.2 关注/取关作者
- **接口路径**: `/api/v1/interactions/follow`
- **请求方式**: `POST`
- **描述**: 关注或取关某位创作者，支持幂等切换。
- **权限要求**: 登录用户

**请求参数 (InteractionRequest):** 同上，`targetId` 为作者的用户 ID。

**响应结构 (Map):** `{"followed": true, "message": "关注成功"}`
...
**响应结构 (Page<CommentVO>):**
包含评论基础信息（`commentId`, `content`, `nickname`, `avatar`）及子回复列表 `replies`。
...
| ---------- | -------- | -------- | -------- |
| `targetId` | String   | 是       | 目标模版ID |
| `parentId` | Long     | 否       | 父评论ID，顶级评论传 0 或空 |
| `content`  | String   | 是       | 评论内容文字 |
...
| `page`     | int      | 否       | 页码     |
| `size`     | int      | 否       | 每页记录 |
| `type`     | String   | 否 | SYSTEM / LIKE / FOLLOW / COMMENT / PURCHASE |

**响应结构 (Page<Notification>):**
包含 `title`, `content`, `relatedId`, `isRead` 等。
